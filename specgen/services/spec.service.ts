...
import { 
  ...
  SpecListResponse,
  SpecNotFoundError,
  SpecValidationError as ValidationError
...
export class SpecService {
  private repository: SpecRepository;
  ...
  async createSpec(data: CreateSpecRequest): Promise<Spec> {
    ...
    try {
      ...
      logger.error('Failed to create specification', error as Error, { title: data.title });
      throw error;
    }
  }

  async updateSpec(id: number, data: UpdateSpecRequest): Promise<Spec> {
    ...
    try {
      ...
      logger.error('Failed to update specification', error as Error, { id });
      throw error;
    }
  }

  async getSpec(id: number, includeRelations = false): Promise<Spec> {
    ...
    try {
      ...
      logger.error('Failed to retrieve specification', error as Error, { id });
      throw error;
    }
  }

  async listSpecs(query: ListSpecsQuery = {}): Promise<SpecListResponse> {
    ...
    try {
      ...
      logger.error('Failed to list specifications', error as Error, query);
      throw error;
    }
  }
  ...
  async deleteSpec(id: number): Promise<void> {
    ...
    try {
      ...
      logger.error('Failed to delete specification', error as Error, { id });
      throw error;
    }
  }

  async getSpecsByStatus(status: SpecStatus): Promise<Spec[]> {
    ...
    try {
      ...
      logger.error('Failed to retrieve specifications by status', error as Error, { status });
      throw error;
    }
  }

  async getSpecStats(): Promise<{ total: number; byStatus: Record<SpecStatus, number> }> {
    ...
    try {
      ...
      logger.error('Failed to retrieve specification statistics', error as Error);
      throw error;
    }
  }

  async searchSpecsByTitle(pattern: string): Promise<Spec[]> {
    ...
    try {
      ...
      logger.error('Failed to search specifications by title', error as Error, { pattern });
      throw error;
    }
  }
}