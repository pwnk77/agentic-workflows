...
export class SearchService {
  ...
  async searchSpecs(query: SearchQuery): Promise<SearchResponse> {
    ...
    if (!validationResult.success) {
      throw new SearchValidationError(
        `Invalid search query: ${validationResult.error.errors.map(e => e.message).join(', ')}`
      );
    }
    ...
    try {
      ...
      logger.error('Search failed', error as Error, { query: query.query });
      throw error;
    }
  }

  async suggestTerms(partialQuery: string, limit = 10): Promise<SearchSuggestion[]> {
    ...
    try {
      ...
      logger.error('Failed to get search suggestions', error as Error, { partialQuery });
      return [];
    }
  }

  async getSearchStats(): Promise<FTSStats> {
    ...
    try {
      ...
      logger.error('Failed to get FTS statistics', error as Error);
      throw error;
    }
  }
  ...
  async optimizeIndex(): Promise<void> {
    ...
    try {
      ...
      logger.error('Failed to optimize FTS index', error as Error);
      throw error;
    }
  }
  ...
  async validateFTSIntegrity(): Promise<boolean> {
    try {
      const dataSource = await getDataSource();
      ...
      logger.error('FTS integrity validation failed', error as Error);
      return false;
    }
  }
}