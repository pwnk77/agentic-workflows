...
export class SpecResourceHandler {
  ...
  async list(): Promise<Array<{ uri: string; name?: string; description?: string; mimeType?: string }>> {
    try {
      ...
      logger.error('Failed to list spec resources', error as Error);
      return [];
    }
  }
  ...
  async read(uri: string): Promise<{ contents: Array<{ uri: string; mimeType?: string; text?: string }> }> {
    try {
      ...
      logger.error('Failed to read spec resource', error as Error, { uri });
      throw error;
    }
  }

  private formatSpecAsMarkdown(spec: any): string {
    const lines = [
      `# ${spec.title}`,
      '',
      `**Status**: ${spec.status}`,
    ...
    if (spec.todos && spec.todos.length > 0) {
      ...
      spec.todos.forEach((todo: any, index: number) => {
    ...
    if (spec.issueLogs && spec.issueLogs.length > 0) {
      ...
      spec.issueLogs.forEach((log: any) => {
