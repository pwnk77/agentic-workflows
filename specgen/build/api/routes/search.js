...
searchRouter.get('/', validate({ query: searchQuerySchema }), asyncHandler(async (req, res) => {
    await searchController.searchSpecs(req, res);
}));
// GET /api/search/suggestions - Get search suggestions
searchRouter.get('/suggestions', validate({ query: suggestionsQuerySchema }), asyncHandler(async (req, res) => {
    await searchController.getSuggestions(req, res);
}));
// GET /api/search/stats - Get search statistics
searchRouter.get('/stats', asyncHandler(async (req, res) => {
    await searchController.getSearchStats(req, res);
}));
// POST /api/search/optimize - Optimize search index
searchRouter.post('/optimize', asyncHandler(async (req, res) => {
    await searchController.optimizeIndex(req, res);
}));
// GET /api/search/integrity - Validate FTS integrity
searchRouter.get('/integrity', asyncHandler(async (req, res) => {
