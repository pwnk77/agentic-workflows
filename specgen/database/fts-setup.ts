...
export async function rebuildFTSIndex(dataSource: DataSource, options: FTSRebuildOptions = {}): Promise<void> {
  try {
    ...
    logger.error('Failed to rebuild FTS index:', error as Error);
    throw error;
  }
}

export async function optimizeFTSIndex(dataSource: DataSource): Promise<void> {
  try {
...
export async function getFTSStatus(dataSource: DataSource): Promise<any> {
  try {
    ...
    logger.error('Failed to get FTS status:', error as Error);
    throw error;
  }
}

export async function validateFTSIntegrity(dataSource: DataSource): Promise<boolean> {
  try {
