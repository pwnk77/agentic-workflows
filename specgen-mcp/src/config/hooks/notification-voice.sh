...
if echo "$INPUT" | jq -e . >/dev/null 2>&1; then
    ...
    case "$EVENT_TYPE" in
        "Stop")
            # Check if another Stop hook is already active to prevent recursion
            STOP_HOOK_ACTIVE=$(echo "$INPUT" | jq -r '.stop_hook_active // false')
            if [ "$STOP_HOOK_ACTIVE" = "true" ]; then
                # Another Stop hook is already active, skip to prevent recursion
                exit 0
            fi
            ...
            # Generate summary using claude -p with context (recursion prevented by stop_hook_active check above)
            if [ -n "$CONTEXT" ]; then
