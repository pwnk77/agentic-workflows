import { listSpecsTool, getSpecTool, dbMetricsTool } from '../tools/file-spec-tools.js';

/**
 * Resource handler for individual specifications (file-based)
 */
export class SpecResourceHandler {
  ...
  static async listSpecResources(): Promise<Array<{
    ...
    try {
      const result = await listSpecsTool.handler({ limit: 100 });
      
      if (!result.success || !result.specs) {
        return [];
      }
      
      return result.specs.map((spec: any) => ({
        uri: this.getSpecUri(spec.id),
        name: spec.title,
        description: `${spec.status} specification in ${spec.category || 'general'} category`,
  ...
  static async readSpecResource(uri: string): Promise<{
    ...
    const result = await getSpecTool.handler({ spec_id: specId });
...
/**
 * Resource handler for project information (file-based)
 */
export class ProjectResourceHandler {
  ...
  static async listProjectResources(): Promise<Array<{
    ...
    try {
      const statsResult = await dbMetricsTool.handler({});
      const stats = statsResult.success ? statsResult : { total: 0 };
      
      return [
        {
          uri: 'spec://project/info',
          name: 'Project Information',
          description: 'Information about file-based project',
  ...
  private static async generateProjectSummary(): Promise<string> {
    ...
    Object.entries(stats.byStatus).forEach(([status, count]) => {
      ...
      const percentage = stats.total > 0 ? (((count as number) / stats.total) * 100).toFixed(1) : '0';
      summary += `- ${icon} **${status}**: ${count} (${percentage}%)\n`;
    });
    
    if (stats.byGroup && Object.keys(stats.byGroup).length > 0) {
      summary += `\n### By Feature Group\n\n`;
