...
export const updateSpecTool: any = {
  ...
  handler: async (args: any) => {
    ...
    try {
      // Serialize related_specs array to JSON string for database consistency
      if (updates.related_specs !== undefined && Array.isArray(updates.related_specs)) {
        (updates as any).related_specs = JSON.stringify(updates.related_specs);
      }
      
      // Enhanced error handling with retry logic for database operations
      let retryCount = 0;
      const maxRetries = 1;
      ...
      while (retryCount <= maxRetries) {
        try {
          ...
          return {
            success: true,
            spec,
            message: `Updated specification "${spec.title}"${retryCount > 0 ? ' (after retry)' : ''}`
          };
        } catch (dbError: any) {
          // Handle database corruption specifically
          if (dbError.message && dbError.message.includes('malformed')) {
            console.error(`[updateSpec] Database corruption detected for spec ${spec_id}, attempt ${retryCount + 1}`);
            
            if (retryCount < maxRetries) {
              console.log(`[updateSpec] Attempting recovery and retry for spec ${spec_id}`);
              
              // Try to recover by performing maintenance
              try {
                const { EnhancedDatabaseConnection } = await import('../../database/enhanced-connection.js');
                EnhancedDatabaseConnection.checkpointAll();
                console.log(`[updateSpec] Database maintenance completed, retrying...`);
                retryCount++;
                continue; // Retry the operation
              } catch (maintenanceError) {
                console.error(`[updateSpec] Database maintenance failed:`, maintenanceError);
              }
            }
            
            return {
              success: false,
              error: 'Database corruption detected. Please run database health check and maintenance operations.',
              corruption_detected: true,
              suggested_action: 'Run mcp__specgen-mcp__db_health_check followed by mcp__specgen-mcp__db_maintenance',
      ...
      throw new Error('Update operation exceeded maximum retry attempts');
  ...
  handler: async (args: any) => {
    ...
    try {
      ...
      try {
        ...
        // Handle database corruption specifically
        if (dbError.message && dbError.message.includes('malformed')) {
          console.error(`[updateSpecRelationships] Database corruption detected for spec ${spec_id}`);
          return {
            success: false,
            error: 'Database corruption detected. Please run database health check and maintenance operations.',
            corruption_detected: true,
